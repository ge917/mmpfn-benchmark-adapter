...
from mmpfn.models.mmpfn.constants import (
...
from mmpfn.models.mmpfn.model.loading import download_model, load_model

if TYPE_CHECKING:
    from sklearn.base import TransformerMixin
    from sklearn.pipeline import Pipeline

    from mmpfn.models.mmpfn import MMPFNClassifier, XType, YType
    from mmpfn.models.mmpfn.model.config import InferenceConfig
...
def validate_Xy_fit(
    X: XType,
    y: YType,
    estimator: MMPFNRegressor | MMPFNClassifier,
    *,
    ...
    # scikit-learn removed BaseEstimator._validate_data in 1.7.  Prefer it
    # where available, but keep the same validation contract on newer sklearn.
    if hasattr(estimator, "_validate_data"):
        X, y = estimator._validate_data(
            X, y, accept_sparse=False, dtype=None,
            force_all_finite="allow-nan", ensure_min_samples=2,
            ensure_min_features=1, y_numeric=ensure_y_numeric,
            estimator=estimator,
        )
    else:
        ...
        X, y = check_X_y(
            X, y, accept_sparse=False, dtype=None,
            ensure_all_finite="allow-nan", ensure_min_samples=2,
            ensure_min_features=1, y_numeric=ensure_y_numeric,
        )
...
def validate_X_predict(
    ...
    if hasattr(estimator, "_validate_data"):
        return estimator._validate_data(  # type: ignore
            X, reset=False, accept_sparse=False, dtype=None,
            force_all_finite="allow-nan", estimator=estimator,
        )
    checked = check_array(
        X, accept_sparse=False, dtype=None, ensure_all_finite="allow-nan",
    )
    if checked.shape[1] != estimator.n_features_in_:
        raise ValueError(
            f"X has {checked.shape[1]} features, but MMPFN was fitted with "
