...
def fine_tune_mmpfn(
    *,
    ...
    image_val: np.ndarray | None = None,
    ...
    Saves the best model based on the validation loss to disk under
    `save_path_to_fine_tuned_model`. When `save_all_checkpoints_dir` is supplied,
    also saves the initial model and every validated fine-tuning step, together with
    a CSV of their validation metrics.
    ...
    if not create_val_data:
        n_samples += len(X_val) if X_val is not None else len(image_val)
    else:
    ...
    checkpoint_records: list[dict[str, float | int | str | bool]] = []
    ...
    _save_checkpoint(save_path_to_fine_tuned_model)
    if save_all_checkpoints_dir is not None:
        initial_checkpoint = save_all_checkpoints_dir / "step_0000.ckpt"
        _save_checkpoint(initial_checkpoint)
        checkpoint_records.append(
            {
                "step": 0,
                "checkpoint": str(initial_checkpoint),
                "validation_loss": float(best_validation_loss),
                "validation_score": float(best_validation_score),
                "is_best_so_far": True,
    ...
    for step_i, batch_data in iter_steps_pbar:
        ...
        if validate_now:
            ...
            if is_best:
                ...
                _save_checkpoint(save_path_to_fine_tuned_model)
            if save_all_checkpoints_dir is not None:
                checkpoint_path = save_all_checkpoints_dir / f"step_{step_i:04d}.ckpt"
    ...
    if save_all_checkpoints_dir is not None:
        pd.DataFrame(checkpoint_records).to_csv(
            save_all_checkpoints_dir / "checkpoint_metrics.csv", index=False
        )
